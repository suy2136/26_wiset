...
ADALORA_INIT_RANK="${ADALORA_INIT_RANK:-}"
...
use_v19_schedule() {
  EPOCHS=4
  CHECKPOINT_INTERVAL=8820
  VALIDATION_INTERVAL=4410
  EVAL_PROGRESS_INTERVAL=500
...
if [[ "$VARIANT" == "nbs" || "$VARIANT" == "nbs_v2" || \
  ...
  if [[ "$VARIANT" == "nbs_v2" ]]; then
  ...
  elif [[ "$VARIANT" == "nbs_v23" ]]; then
    MODEL_TAG="llama_base_low_rank_adalora_nbs_v23"
    DISPLAY_NAME="NBS-NetLLM v23 (min2-max32-budget320, mean-rank5, seed1)"
    RANK_CONFIG="configs/adalora_rank_config_llama7b_min2_max32.json"
    RANK_BUDGET=320
    SEED=1
    EARLY_STOPPING_PATIENCE=2
    EARLY_STOPPING_MIN_DELTA=0.0001
    EXPERIMENT_ARGS=(
      --experiment-tag nbs_v23
      --early-stopping-patience "$EARLY_STOPPING_PATIENCE"
      --early-stopping-min-delta "$EARLY_STOPPING_MIN_DELTA"
    )
  elif [[ "$VARIANT" == "nbs_v24" ]]; then
    use_v19_schedule
    MODEL_TAG="llama_base_low_rank_adalora_nbs_v24"
    DISPLAY_NAME="NBS-NetLLM v24 (v19 conditions: min2-max32-budget512, seed2)"
    RANK_CONFIG="configs/adalora_rank_config_llama7b_min2_max32.json"
    RANK_BUDGET=512
    SEED=2
    EARLY_STOPPING_PATIENCE=2
    EARLY_STOPPING_MIN_DELTA=0.0001
    EXPERIMENT_ARGS=(
      --experiment-tag nbs_v24
      --early-stopping-patience "$EARLY_STOPPING_PATIENCE"
      --early-stopping-min-delta "$EARLY_STOPPING_MIN_DELTA"
    )
  elif [[ "$VARIANT" == "nbs_v25" ]]; then
    use_v19_schedule
    MODEL_TAG="llama_base_low_rank_adalora_nbs_v25"
    DISPLAY_NAME="NBS-NetLLM v25 (v19 conditions: min2-max32-budget512, seed3)"
    RANK_CONFIG="configs/adalora_rank_config_llama7b_min2_max32.json"
    RANK_BUDGET=512
    SEED=3
    EARLY_STOPPING_PATIENCE=2
    EARLY_STOPPING_MIN_DELTA=0.0001
    EXPERIMENT_ARGS=(
      --experiment-tag nbs_v25
      --early-stopping-patience "$EARLY_STOPPING_PATIENCE"
      --early-stopping-min-delta "$EARLY_STOPPING_MIN_DELTA"
    )
  elif [[ "$VARIANT" == "nbs_v27" ]]; then
    use_v19_schedule
    MODEL_TAG="llama_base_low_rank_adalora_nbs_v27"
    DISPLAY_NAME="NBS-NetLLM v27 adaptive (min2-max32, floor128-cap512, tau0.05, seed1)"
    RANK_CONFIG="configs/adalora_rank_config_llama7b_min2_max32.json"
    RANK_BUDGET=512
    SEED=1
    LORA_SEED=1
    ...
    EARLY_STOPPING_MIN_DELTA=0.0001
    EXPERIMENT_ARGS=(
      --experiment-tag nbs_v27
      --early-stopping-patience "$EARLY_STOPPING_PATIENCE"
      --early-stopping-min-delta "$EARLY_STOPPING_MIN_DELTA"
    )
  elif [[ "$VARIANT" == "nbs_v28" ]]; then
    use_v19_schedule
    MODEL_TAG="llama_base_low_rank_adalora_nbs_v28"
    DISPLAY_NAME="NBS-NetLLM v28 fixed-shadow ablation (min2-max32-budget512, active-only, seed1)"
    RANK_CONFIG="configs/adalora_rank_config_llama7b_min2_max32.json"
    RANK_BUDGET=512
    SEED=1
    LORA_SEED=1
    ...
    EARLY_STOPPING_MIN_DELTA=0.0001
    EXPERIMENT_ARGS=(
      --experiment-tag nbs_v28
      --early-stopping-patience "$EARLY_STOPPING_PATIENCE"
      --early-stopping-min-delta "$EARLY_STOPPING_MIN_DELTA"
    )
  elif [[ "$VARIANT" == "nbs_v29" ]]; then
    use_v19_schedule
    MODEL_TAG="llama_base_low_rank_adalora_nbs_v29"
    DISPLAY_NAME="NBS-NetLLM v29 ultra-low-budget (min2-max32-budget192, mean-rank3, seed1)"
    RANK_CONFIG="configs/adalora_rank_config_llama7b_min2_max32.json"
    RANK_BUDGET=192
    SEED=1
    LORA_SEED=1
    ...
    EARLY_STOPPING_MIN_DELTA=0.0001
    EXPERIMENT_ARGS=(
      --experiment-tag nbs_v29
      --early-stopping-patience "$EARLY_STOPPING_PATIENCE"
      --early-stopping-min-delta "$EARLY_STOPPING_MIN_DELTA"
    )
  elif [[ "$VARIANT" == "nbs_v19_data2" ]]; then
    use_v19_schedule
    MODEL_TAG="llama_base_low_rank_adalora_nbs_v19_data2"
    DISPLAY_NAME="NBS-NetLLM v19 data-seed ablation (min2-max32-budget512, LoRA seed1, data seed2)"
    RANK_CONFIG="configs/adalora_rank_config_llama7b_min2_max32.json"
    RANK_BUDGET=512
    SEED=1
    LORA_SEED=1
    ...
    EARLY_STOPPING_MIN_DELTA=0.0001
    EXPERIMENT_ARGS=(
      --experiment-tag nbs_v19_data2
      --early-stopping-patience "$EARLY_STOPPING_PATIENCE"
      --early-stopping-min-delta "$EARLY_STOPPING_MIN_DELTA"
    )
  elif [[ "$VARIANT" == "nbs_budget256_seed1" ]]; then
    use_v19_schedule
    MODEL_TAG="llama_base_low_rank_adalora_nbs_budget256_seed1"
    DISPLAY_NAME="NBS-NetLLM low-budget ablation (min2-max32-budget256, mean-rank4, seed1)"
    RANK_CONFIG="configs/adalora_rank_config_llama7b_min2_max32.json"
    RANK_BUDGET=256
    SEED=1
    LORA_SEED=1
    ...
    EARLY_STOPPING_MIN_DELTA=0.0001
    EXPERIMENT_ARGS=(
      --experiment-tag nbs_budget256_seed1
      --early-stopping-patience "$EARLY_STOPPING_PATIENCE"
      --early-stopping-min-delta "$EARLY_STOPPING_MIN_DELTA"
    )
  ...
  if [[ "$VARIANT" == "eva_b512_data2" || "$VARIANT" == "eva_b512_data1" ]]; then
    use_v19_schedule
    ...
    SEED=1
    LORA_SEED=1
    if [[ "$VARIANT" == "eva_b512_data1" ]]; then
      DATA_SEED=1
      DISPLAY_NAME="EVA-NetLLM (min2-max32-budget512, LoRA seed1, data seed1)"
    else
      DATA_SEED=2
...
elif [[ "$VARIANT" == "uniform_r8_data2" || "$VARIANT" == "uniform_r8_data1" ]]; then
  use_v19_schedule
  MODEL_TAG="llama_base_low_rank_${VARIANT}"
  RANK=8
  RANK_BUDGET=512
  SEED=1
  LORA_SEED=1
  if [[ "$VARIANT" == "uniform_r8_data1" ]]; then
    DATA_SEED=1
    DISPLAY_NAME="Uniform LoRA (rank8, budget512, LoRA seed1, data seed1)"
  else
    DATA_SEED=2
  ...
  EXTRA_ARGS=(
    ...
    --early-stopping-min-delta "$EARLY_STOPPING_MIN_DELTA"
        ...
        "$VARIANT" == "adalora_b512_data1" ]]; then
  use_v19_schedule
  ...
  if [[ "$VARIANT" == "adalora_b512_data1" ]]; then
    DATA_SEED=1
    DISPLAY_NAME="Stock PEFT AdaLoRA (init32-target8-budget512, LoRA seed1, data seed1)"
  else
    DATA_SEED=2
  ...
  EXTRA_ARGS=(
    --use-adalora
    --adalora-allocator peft
    --adalora-init-rank "$ADALORA_INIT_RANK"
    --adalora-allocation-interval "$ADALORA_ALLOCATION_INTERVAL"
    --experiment-tag "$VARIANT"
    --early-stopping-patience "$EARLY_STOPPING_PATIENCE"
    --early-stopping-min-delta "$EARLY_STOPPING_MIN_DELTA"
  ...
  SHAPLEY_VALUE_MODE="${SHAPLEY_VALUE_MODE:-teacher-forcing}"
  EARLY_STOPPING_PATIENCE=2
  EARLY_STOPPING_MIN_DELTA=0.0001
  EXTRA_ARGS=(
    --use-adalora
