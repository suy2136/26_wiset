...
VP_FP16_PRESCALED_QK="${VP_FP16_PRESCALED_QK:-0}"
...
if [[ "$VP_FP16_PRESCALED_QK" != "0" && "$VP_FP16_PRESCALED_QK" != "1" ]]; then
  echo "VP_FP16_PRESCALED_QK must be 0 or 1."
  exit 2
fi
...
if [[ "$VP_FP16_PRESCALED_QK" == "1" ]]; then
  TRAIN_CMD+=(--vp-fp16-prescaled-qk)
fi
