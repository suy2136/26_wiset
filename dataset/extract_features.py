...
def extract_vit_features(img, model=None):
    '''
    ...
    :param model: (Optional) a pre-loaded vit_b_16 model. Pass this in when extracting
        features for many images so the (slow) pretrained-weight load only happens once.
        If None, a new model is loaded on every call (matches the original behavior).
    '''
