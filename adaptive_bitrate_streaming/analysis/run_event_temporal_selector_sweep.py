"""Evaluate an existing ABR NBS-v19 checkpoint with event-aware selection.

The checkpoint is never trained or modified.  NBS-only and event-aware K=1..4
use the same seed, traces, model weights, and FP16 inference conditions, while
recent-timestep selection and speculative decoding remain disabled.
"""
...
DEFAULT_OUTPUT = RESULTS_ROOT / 'event_temporal_selector_sweep.csv'


def configurations(event_counts):
    yield {'experiment': 'nbs_only', 'selector': 'none', 'max_events': 0}
    for count in event_counts:
        yield {
            'experiment': f'event_temporal_k{count}',
            'selector': 'event-aware',
            'max_events': count,
        }
...
def validate_checkpoint(checkpoint_dir, rank_budget):
    ...
    if metadata.get('seed') != 1:
        raise ValueError('event temporal sweep requires a seed-1 checkpoint')
...
def build_command(args, experiment):
    ...
    if experiment['selector'] == 'event-aware':
        command.extend([
...
def run_signature(args, event_counts):
    return {
...
def write_results(rows, output, signature, requested_experiments):
    rows = add_baseline_comparisons(rows)
    output.parent.mkdir(parents=True, exist_ok=True)
    preferred = [
        'experiment', 'seed', 'checkpoint_dir', 'rank_budget',
        'mean_reward', 'mean_reward_delta_vs_nbs', 'qoe_raw_mean',
        ...
        'selected_tokens_mean', 'token_reduction_ratio', 'selector_calls',
        'event_timesteps_selected', 'event_timesteps_selected_mean',
        'rebuffer_events_selected', 'throughput_change_events_selected',
        'low_buffer_events_selected', 'bitrate_switch_events_selected',
        'latest_history_steps_preserved', 'target_plm_calls',
        'llm_call_reduction_ratio', 'time', 'metrics_path',
    ]
...
def main():
    ...
    parser.add_argument('--event-counts', type=int, nargs='+', default=[1, 2, 3, 4])
    ...
    if any(count <= 0 for count in args.event_counts):
        parser.error('--event-counts values must be positive')
    if len(set(args.event_counts)) != len(args.event_counts):
        parser.error('--event-counts values must be unique')

    experiments = list(configurations(args.event_counts))
    signature = run_signature(args, args.event_counts)
    metadata = {}
    if not args.dry_run:
    ...
    for experiment in experiments:
        if experiment['experiment'] in completed:
            print(
                f"[{experiment['experiment']}] already complete; skipping",
                flush=True,
            )
            continue
        command = build_command(args, experiment)
        print(f"[{experiment['experiment']}] {shlex.join(command)}", flush=True)
        if args.dry_run:
            continue
