...
def load_plm(model_name, model_path, specials_to_add = None, **kwargs):
    ...
    # optional: stream weights directly in this dtype (e.g. torch.float16) instead of
    # loading fp32 and downcasting after -- avoids a transient full-fp32 memory spike
    # during from_pretrained, which matters for large models loaded via device_map.
    ...
    if 'llama' in model_name and device_input_side is not None and device_output_side is not None:
        ...
        model = model_class.model.from_pretrained(model_path, device_map=device_map, **from_pretrained_kwargs)
    else:
