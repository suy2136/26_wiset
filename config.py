...
class Config:
    ...
    dataset_images = {
        # raw extracted video frames (jpg). Wu2017 has no frames available, so the
        # patch-selection multimodal work only supports Jin2022 for now.
        'Jin2022':  _base_dir + 'data/images/Jin2022_images',
        'Wu2017':  _base_dir + 'data/images/Wu2017images/saliencyMap'
    }
    dataset_image_features = {
        # offline-cached ViT CLS-token features (.pth), built by dataset/extract_features_cache.py
    ...
    # per-video actual frame count manifest, generated by dataset/extract_features_cache.py
    # (a few Jin2022 videos have fewer frames than the nominal 1500/1800, see dataset_short_frame_videos)
    dataset_frame_count_manifest = {
        'Jin2022': _base_dir + 'data/images/Jin2022_images/frame_counts.json',
    }
    # videos whose actual frame count falls short of the nominal count for their group
    # (video9/27: nominal 1800, video18: nominal 1500) -> frame-index lookups near the
    # tail of these videos must clamp to the last available frame instead of erroring out
    dataset_short_frame_videos = {
        'Jin2022': [9, 18, 27],
    }
    plms_dir = _base_dir + ('../../downloaded_plms' if 'viewport_prediction' in os.getcwd() else '../downloaded_plms')
    ...
    default_patch_grid = (4, 4)  # (rows, cols) grid used to split a frame for patch selection
